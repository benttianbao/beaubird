...
const PREFLIGHT_FILES = Object.freeze([
  "tools/preflight-zhejiang-spatial-profile-calibration.js",
  "server/prediction/spatial-profile-calibration-candidate.js",
  "tools/score-zhejiang-spatial-profile-calibration.js"
]);

class SpatialProfilePreflightError extends Error {
  constructor(code, message, details = undefined) {
    super(message);
    this.name = "SpatialProfilePreflightError";
    this.code = code;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    const value = () => {
      ...
      if (index >= argv.length) {
        throw new SpatialProfilePreflightError(
          "SPATIAL_PROFILE_PREFLIGHT_OPTIONS_INVALID",
          `${argument} 缺少值。`
        );
      }
      return argv[index];
...
function usage() {
  return "用法：node tools/preflight-zhejiang-spatial-profile-calibration.js --neighbor-cache <v9-r2.sqlite> --feature-cache <v10.sqlite> --v10-report <v10.report.json> --snapshot <snapshot.sqlite> --features <features.json> --spatial-split-manifest <split.json> --preregistration <v11.json> --default-model <default.sqlite> --output <preflight.json>";
}
...
function expectedScoringCommand(preregistration) {
  return [
    "node tools/score-zhejiang-spatial-profile-calibration.js",
...
function assertFrozenCommand(preregistration) {
  ...
  if (preregistration?.commands?.candidateScoring !== expected) {
    throw new SpatialProfilePreflightError(
      "SPATIAL_PROFILE_COMMAND_MISMATCH",
      "v11 唯一候选评分命令与冻结路径不一致。",
      {
        expected,
...
function run(options) {
  ...
  if (
    ...
    throw new SpatialProfilePreflightError(
      "SPATIAL_PROFILE_PREFLIGHT_OUTPUT_EXISTS",
      "v11 预检输出或 SHA sidecar 已存在。"
    );
  }
  ...
  if (
    ...
    throw new SpatialProfilePreflightError(
      "SPATIAL_PROFILE_DIAGNOSTIC_OUTPUT_EXISTS",
      "v11 候选诊断产物已存在，禁止覆盖。"
    );
  }
  ...
  try {
    ...
    if (defaultModelSha256 !== DEFAULT_MODEL_SHA256) {
      throw new SpatialProfilePreflightError(
        "SPATIAL_PROFILE_DEFAULT_MODEL_CHANGED",
        "默认离线模型 SHA 已改变。"
      );
    }
    ...
    const report = {
      schemaVersion: 1,
      kind: "zhejiang_spatial_profile_calibration_v11_preflight",
      ok: true,
  ...
  try {
    ...
    process.stderr.write(
      `${JSON.stringify({
        ...
        code:
          error.code ||
          "SPATIAL_PROFILE_PREFLIGHT_FAILED",
