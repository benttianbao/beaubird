...
- eBird 浙江当季分析：固定 `CN-ZJ`，按目标日期前后窗口统计多年历史记录，推算当季可能出现鸟种。
...
在 eBird 模块的“浙江当季可能出现鸟种”中，选择目标日期、历史年份数和前后天数后点击分析。功能固定使用 `CN-ZJ`，会读取多年同日期窗口的历史记录，并结合最近 30 天浙江记录标记是否已经确认出现。
